...
export async function POST(req: NextRequest) {
  try {
    ...
    const batteryId = bodyObj.id || bodyObj.batteryId || bodyObj.BatteryID;
    
    if (!batteryId) {
      return NextResponse.json({ success: false, message: 'Battery ID is required' }, { status: 400 });
    }
    
    const url = `${API_URL}/Batteries/${batteryId}`;
