...
export class AuthRepository implements IAuthRepository {
  ...
  async login(credentials: LoginCredentials): Promise<AuthResponse> {
    ...
    // Extract stationId and stationName from login response
    let stationId = userData.stationId || userData.StationID || userData.stationID || userData.station_id;
    const stationName = userData.stationName || userData.StationName || userData.station_name;
    
    // If StationID not in login response, try to get it from JWT token claim
    // Backend sets StationID claim in JWT token (see AuthService.GenerateJwtToken)
    if (!stationId && accessToken && typeof window !== 'undefined') {
      try {
        const parts = accessToken.split('.');
        if (parts.length >= 2) {
          const payload = JSON.parse(atob(parts[1]));
          const tokenStationID = payload.StationID || payload.stationID;
          // Backend may set "null" as string if StationID is null
          if (tokenStationID && tokenStationID !== 'null' && tokenStationID !== '') {
            stationId = tokenStationID;
            console.log('[AuthRepository] Got StationID from JWT token claim:', stationId);
          }
        }
      } catch (e) {
        console.warn('[AuthRepository] Could not extract StationID from token:', e);
      }
    }
  ...
  async getCurrentUser(): Promise<AuthUser> {
    try {
      // Use Next.js API route to handle cookies and proxy to backend
    ...
    if (!roleValue && (data.roleID || data.RoleID || data.roleId || data.RoleId)) {
      ...
      // Fetch errors don't have error.response, they throw directly
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes('Network') || errorMessage.includes('Failed to fetch')) {
          console.warn('[AuthRepository] getCurrentUser network error:', errorMessage);
          throw new Error('Network error');
        }
        
        // Re-throw with original message
        console.error('[AuthRepository] getCurrentUser failed:', errorMessage);
        throw error;
      }
      
      // Fallback for unknown error types
      console.error('[AuthRepository] getCurrentUser failed:', error);
