    ...
    if (stations && stations.length > 0) {
      ...
      console.log('[StationService] Sample stations (first 3):', 
        stations.slice(0, 3).map((s: any) => ({
          id: s.id || s.stationId || s.stationID || s.stationid,
      ...
      if (exactMatch) {
        const id = exactMatch.id || exactMatch.stationId || exactMatch.stationID || exactMatch.stationid;
        if (id) {
      ...
      if (partialMatch) {
        const id = partialMatch.id || partialMatch.stationId || partialMatch.stationID || partialMatch.stationid;
        if (id) {
